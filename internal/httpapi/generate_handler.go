...
func (s *Server) RerunToday(w http.ResponseWriter, r *http.Request) {
	slog.Info("generate rerun request received")
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Minute)
	defer cancel()

	result, err := s.generator.RerunToday(ctx)
	if err != nil {
		slog.Warn("generate rerun request failed", "error", err.Error())
		writeJSON(w, statusForGenerateError(err), errorResponse{Success: false, Error: err.Error()})
		return
	}
	slog.Info("generate rerun request succeeded", "date", result.Date, "file", result.File)
	writeJSON(w, http.StatusOK, generateResponse{
		Success: true,
		Date:    result.Date,
		File:    result.File,
